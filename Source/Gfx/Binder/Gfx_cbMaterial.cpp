...
//===== クラス実装 =====
GfxCBuffMaterial::GfxCBuffMaterial(GfxMain& gfx, GfxHeapMgr& heapMgr, const GfxMaterialData& material) :
    GfxBinder(), m_material(material)
{
    //定数バッファ初期化
    if (!m_pPcBuff)
        m_pPcBuff = std::make_unique<pcbMta>(gfx, heapMgr, static_cast<UINT>(CB_SLOT_PS::MATERIAL));
}
