...
WinWindowClass::WinWindowClass() : m_hAppInst(GetModuleHandle(nullptr))
{
    ...
    windowClass.lpfnWndProc = WinDirectX::WndProc_Init;
...
//コンストラクタ
WinDirectX::WinDirectX(LPCWSTR windowName, int nWndWidth, int nWndHeight, int nWndPosX, int nWndPosY) :
    WinWindow(), m_posX(nWndPosX), m_posY(nWndPosY), m_width(nWndWidth), m_height(nWndHeight),
...
//デストラクタ
WinDirectX::~WinDirectX() noexcept(false)
{
...
//トランスフォーム
void WinDirectX::Transform(int nWndPosX, int nWndPosY, int nWndWidth, int nWndHeight)
{
    //サイズ取得
...
//タイトル出力
void WinDirectX::TitlePrint(const std::string& text) const
{
...
//タイトル出力（マウス座標）
void WinDirectX::TitlePrint_MousePos() const
{
    //メッセージボックス表示バグあり
...
//タイトル出力（ホイール値）
void WinDirectX::TitlePrint_WheelVal()
{
...
//マウス使用・不使用
void WinDirectX::EnableCursor() noexcept
{
    m_bDrawCursor = true;
    ShowCursor();
    UnlockCursor();
...
void WinDirectX::DisableCursor() noexcept
{
    m_bDrawCursor = false;
    HideCursor();
    LockCursor();
...
//マウス使用状態確認
bool WinDirectX::IsUsingCursor() const noexcept
{
    return m_bDrawCursor;
}

//WndProc初期化
LRESULT CALLBACK WinDirectX::WndProc_Init(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept
{
    //Instance作成時
    if (uMsg == WM_NCCREATE)
    {
        ...
        WinDirectX* const pGameWnd =
            static_cast<WinDirectX*>(pCreateInfo->lpCreateParams);
        ...
        SetWindowLongPtr(hWnd, GWLP_WNDPROC,
            reinterpret_cast<LONG_PTR>(&WinDirectX::WndProc_Call));
...
//WndProc呼び出し
LRESULT CALLBACK WinDirectX::WndProc_Call(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept
{
    //Instanceポインタを取得
    WinDirectX* const pGameWnd =
        reinterpret_cast<WinDirectX*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
...
//WndProc本処理
LRESULT WinDirectX::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept
{
...
//マウス非表示・表示
void WinDirectX::HideCursor() noexcept
{
    while (::ShowCursor(FALSE) >= 0);
}

void WinDirectX::ShowCursor() noexcept
{
    while (::ShowCursor(TRUE) < 0);
}

//マウスをウィンドウ内にロック・ロック解除
void WinDirectX::LockCursor() noexcept
{
    RECT rect;
...
void WinDirectX::UnlockCursor() noexcept
{
    ClipCursor(nullptr);
