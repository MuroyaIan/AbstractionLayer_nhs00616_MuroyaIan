...
WinWindowClass::WinWindowClass() : m_hAppInst(GetModuleHandle(nullptr))
{
    ...
    windowClass.hIcon = LoadIcon(m_hAppInst, MAKEINTRESOURCE(IDI_ABSTRACTIONLAYERNHS00616MUROYAIAN));   //アイコン
    windowClass.hCursor = LoadCursor(m_hAppInst, IDC_ARROW);                                            //カーソル
    windowClass.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
    windowClass.lpszMenuName = nullptr;
    windowClass.lpszClassName = CLASS_NAME;                                     //クラス名
    windowClass.hIconSm = LoadIcon(m_hAppInst, MAKEINTRESOURCE(IDI_SMALL));     //小アイコン
...
WinWindow::WinWindow(LPCWSTR windowName, int nWndWidth, int nWndHeight, int nWndPosX, int nWndPosY) :
    m_posX(nWndPosX), m_posY(nWndPosY), m_width(nWndWidth), m_height(nWndHeight), m_hWindow(nullptr),
    m_bDrawCursor(true), m_rawBuffer(0), m_keyboard(), m_mouse()
{
    //サイズ算出
    DWORD dwExStyle = 0;
...
LRESULT CALLBACK WinWindow::WndProc_Init(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept
{
    ...
    if (uMsg == WM_NCCREATE)
    {
        ...
        WinWindow* const pGameWnd = static_cast<WinWindow*>(pCreateInfo->lpCreateParams);
        ...
        SetWindowLongPtr(hWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(&WinWindow::WndProc_Call));
...
{
    //Instanceポインタを取得
    WinWindow* const pGameWnd = reinterpret_cast<WinWindow*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
